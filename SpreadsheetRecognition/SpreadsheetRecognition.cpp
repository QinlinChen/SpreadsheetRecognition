...
void SpreadsheetRecognition::execute(const SpreadsheetRecognitionParameters &para) {
    ...
    GaussianBlur(mSrcGray, mSrcGray, cv::Size(ks, ks), 0, 0);
    Canny(mSrcGray, mSrcGray, para.cannyThreshold, para.cannyThreshold * para.cannyRatio, para.cannyApertureSize, true);
#ifdef DEBUG
...
#endif
    probFilterHLines(para.probFilterLinesTryCount, para.probFilterLinesExpectation);
#ifdef DEBUG
...
void SpreadsheetRecognition::probFilterHLines(int tryCount, double expectation) {
